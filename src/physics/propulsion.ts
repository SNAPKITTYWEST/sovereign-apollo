...
export interface EngineSpec {
  name:string; thrustN:number; ispS:number; throttleMin:number; throttleMax:number;
}
...
export function throttleCommand(aCmd:number, fc:number, mass:number): number {
...
export function massFlow(thrust:number, isp:number): number { return thrust/(isp*9.80665); }
